...
CREATE TABLE IF NOT EXISTS `metadata_business_system` (
  ...
  `database_name` varchar(128) DEFAULT NULL COMMENT '默认物理库(MySQL schema)，物理DDL默认落库',
  `is_enabled` tinyint NOT NULL DEFAULT 1 COMMENT '1启用 0停用',
  `create_time` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP,
  `update_time` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
...
CREATE TABLE IF NOT EXISTS `metadata_table` (
  ...
  `business_code` varchar(50) NOT NULL DEFAULT 'DEFAULT' COMMENT '业务系统编码',
  `database_name` varchar(128) DEFAULT NULL COMMENT '物理库名，空则使用业务系统默认库',
  `create_time` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP,
  `update_time` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
...
CREATE TABLE IF NOT EXISTS `metadata_field` (
  ...
  `in_form` tinyint NOT NULL DEFAULT 1 COMMENT '1参与表单录入 0不参与（如编码仅列表展示）',
