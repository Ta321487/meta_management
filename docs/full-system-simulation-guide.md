...
|----------|------------------|----------------------|
| **模块类型** | 选下拉里的**中文名称**（须先完成 [第 3 节](#3-模块类型管理) 三条类型） | 表格中的 `moduleType` 为 **typeCode** |
| | `主数据模块类型` | `MASTER_DATA` |
| | `订单业务模块类型` | `ORDER` |
| | `报表与配置模块类型` | `REPORT` |
| **业务系统** | 选 **`演示ERP业务系统`**（须先完成 [第 4 节](#4-业务系统管理)） | `businessCode` = `DEMO_ERP` |
| **排序号** | 手输文档中的数字（新建时界面默认常为 `1`，**请改成表里的值**） | 如 `10`、`20`… |

若你的「模块类型」下拉只有「数据管理型、流程审批型…」而没有「主数据模块类型」：说明尚未执行第 3 节，请先 **模块类型管理** 里按第 3 节新增三条，再回到本节。
